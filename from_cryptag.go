...
// with the given passphrase for use in generating a
// miniLock/session25519 key pair.
func EmailFromPassphrase(passphrase string) string {
	hash := sha512.Sum384([]byte(passphrase))
	return fmt.Sprintf("%x", hash) + "@cryptag.org"
}

// RandomPassphrase uses a capitalized version of the EFF's
// user-friendly wordlist (see
// https://www.eff.org/deeplinks/2016/07/new-wordlists-random-passphrases)
// to generate a random passphrase consisting of numWords words. Each
// additional word contributes 10.34 bits of entropy to the overall
// strength of the resulting passphrase.
func RandomPassphrase(numWords int) (string, error) {
	passphrase := ""
	max := int64(len(wordlist))
	for i := 0; i < numWords; i++ {
		ndx, err := rand.Int(rand.Reader, big.NewInt(max))
		if err != nil {
			return "", err
		}
		passphrase += wordlist[int(ndx.Int64())]
	}
